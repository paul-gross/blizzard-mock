...
- **Drive plane** — a test POSTs these and the driver performs the runner-role call
  ...
  | `POST /_drive/ask` `{chunk_id, question, options?, harness_id?}` | Pushes a `question.asked` fact via `POST {hub}/api/fleet/events`, minting a pollable question hub-side; returns the minted `question_id`. `harness_id` (D8) defaults to the Claude Code compatibility value when omitted |
  | `POST /_drive/poll-answer` `{question_id}` | `GET {hub}/api/fleet/questions/{id}` — the runner's answer poll |
  | `POST /_drive/push-transcript` `{chunk_id, segment_id?, turns?, final?, record_truncated?, harness_id?}` | Pushes one transcript segment record via `POST {hub}/api/fleet/transcripts` (blizzard#246/#247) — the transcript lane's own push, distinct from `/events`; `turns` defaults to one placeholder turn. `harness_id` (D8) defaults to the Claude Code compatibility value when omitted |
  ...
  | `POST /_drive/report-external-usage` `{slug, sampled_at, windows?, name?, raw_slug?}` | Pushes an `external_subscription_usage.sampled` fact via `POST {hub}/api/fleet/events` (issue #218, blizzard#436 phase 3), naming an arbitrary declared subscription's `slug` without waiting on a real runner's sampling cadence — runner-scoped, no `chunk_id`. `raw_slug` is the malformed-slug lever: it replaces `slug` unnarrowed so a test can prove the hub's intake rejects a shape this drive plane would never produce |
  | `POST /_drive/report-external-usage-miss` `{slug, missed_at, reason, name?, raw_slug?}` | Pushes an `external_subscription_usage.missed` fact via `POST {hub}/api/fleet/events` (blizzard#504 D7), the miss-half sibling of `/_drive/report-external-usage` — same runner-scoped, no-`chunk_id`, `raw_slug`-levered shape. `reason` is the sampler's own closed-set miss reason (e.g. `"credential_lapsed"`) — never a token, a refresh token, or a path |
