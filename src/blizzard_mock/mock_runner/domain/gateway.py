...
class IHubGateway(Protocol):
    ...
    #: The dedicated ``POST /chunks/{id}/leases`` route (``LeaseMintReport{epoch,
    #: runner_id}``) — the default transport ``_report_lease`` uses; the
    #: ``lease_via_events`` lever routes the same report through
    #: ``report_lease_via_events`` (the batched ``/events`` push) instead.
