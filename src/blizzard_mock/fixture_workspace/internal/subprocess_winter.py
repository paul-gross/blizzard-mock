...
Invokes the fixture's own ``tools/winter-cli`` via ``mise exec -- uv run``, CWD
pinned there so winter's root-walk resolves the fixture rather than the outer
workspace (pinned by ``tests/test_pin_mock.py``).
"""
